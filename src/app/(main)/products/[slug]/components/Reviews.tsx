...
import { GetProductReviewsDocument, ReviewMediaType, type GetProductReviewsQuery } from "@/gql/graphql";
...
export function Reviews({ productId, slug, variant }: ReviewsProps) {
	const [reviews, setReviews] = useState<GetProductReviewsQuery["getProductReview"]>(null);
	const { tokenData, loading, tokenRefresh } = useUser();
	...
	const fetchReviews = async () => {
		...
		try {
			if (!tokenData && !loading) {
				...
				const token = res.data?.tokenRefresh?.token ?? null;
				if (token) {
					const reviewsData = await executeGraphQL(GetProductReviewsDocument, {
						variables: {
							product: productId,
							status: true,
						},
					...
					setReviews(reviewsData.getProductReview);
				}
			}
		} catch (error) {
	...
	useEffect(() => {
		fetchReviews().catch((error) => console.error("Error in fetchReviews:", error));
	}, [tokenData, loading, tokenRefresh, productId]);
	...
	return (
		<div className="mt-12 bg-gray-100">
			{!isPending ? (
				...
				<div className="flex animate-spin items-center justify-center py-12 text-gray-500">
					<Loader2 size={24} />
				</div>
			)}
		</div>
